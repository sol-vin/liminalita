...
func teleport_in_relation_to(begin : Node3D, end : Node3D):
	self.global_position = end.global_position + (self.global_position - begin.global_position)
	self.on_teleport.emit()
...
func sit(to : Vector3):
	if is_on_floor():
		...
		pre_sit_position = %Head.global_position
...
func stand():
	%Head.global_position = self.pre_sit_position
	self.immobile = false
	self.sitting = false
	on_stand.emit()
...
func _process(_delta):
	...
	if result.has("collider") and !sitting:
		var found_left_click = false
		for child in result.collider.get_children():
				if child is LeftClickAction:
...
func _physics_process(delta): # Most things happen here.
	...
	var actions_input_dir = Input.get_vector(controls.LEFT, controls.RIGHT, controls.FORWARD, controls.BACKWARD)
	
	if !immobile:
		input_dir = actions_input_dir
	if sitting and \
		(Input.is_action_just_pressed(controls.LEFT) or \
		Input.is_action_just_pressed(controls.RIGHT) or \
		Input.is_action_just_pressed(controls.FORWARD) or \
		Input.is_action_just_pressed(controls.BACKWARD)):
		stand()
